...
Bridge provides the `ConfigurationFile`, a simple API to load, reload, save and delete configuration files.
...
### The Patch File
Whenever a resource file is loaded using Bridge's `ConfigurationFile` API, a "_resourceFileName.patch.yml_" file
...
### Config Versions
The versions in the patch file have four digits (`1.2.3.4`). The first three are the semantic version of the Bridge
...
Transforms the data type of existing key into the given Java data type. This will result in a different formatting
